...
public class Util {
	 ...
	 *
	 * @param context
	 *            :Context Incoming current activity instance calling the method
	 * @param msg
	 *            :String To show the display text
	 * @param type
	 *            :int Display Type 1: only OK, 2: "OK", "Cancel" two operations
	 * @param handler
	 *            :Handler Incoming information requires a callback handler can be used as a callback method is to use, msg.what = 1 when the operation is completed status symbol
	 */
	public static void showDialog(Context context, String title, String content, int type, final Handler handler){
		...
		// Setting such content templates
		LayoutInflater inflater = LayoutInflater.from(context);
		View view = inflater.inflate(R.layout.global_dialog_tpl, null);
		Button confirmButton = (Button) view
				.findViewById(R.id.setting_account_bind_confirm);// Confirm
		Button cancelButton = (Button) view
				.findViewById(R.id.setting_account_bind_cancel);// Cancel
		TextView dialogTitle = (TextView) view
				.findViewById(R.id.global_dialog_title);// Title
		View line_hori_center = view.findViewById(R.id.line_hori_center);// The vertical bar
		confirmButton.setVisibility(View.GONE);// Cancel button hidden by default
		...
		dialogTitle.setText(title);// Set Title
		textView.setText(content); // Setting the prompts
		...
		dialog.setCancelable(true);// Return key to close
		dialog.setCanceledOnTouchOutside(true);// Click outside to close
		dialog.show();
	}
